...
export function getOddsProvider(name, { ignoreRuntimeState = false } = {}) {
  if (name) {
  ...
  for (const providerName of config.oddsProviderOrder) {
    if (
      ...
      (ignoreRuntimeState || !isTemporarilyUnavailable(providerName))
    ) {
      return providers[providerName];
    }
  }

  throw new HttpError(503, 'No odds providers are currently available');
}
      ...
      names.map((name) => {
        ...
        return [
          name,
          {
            ...
            temporarilyUnavailable: isTemporarilyUnavailable(name),
